...
async def pandect(user: User = Depends(get_current_user),
    ...
    @cached(f"pandect:{user.id}")
    async def _compute() -> dict:
        return await compute_pandect(db, user.id)

    return {"code": 0, "data": await _compute()}
...
async def comp_forecast_me(user: User = Depends(get_current_user),
    ...
    @cached(f"compme:{user.id}")
    async def _compute():
        return await forecast(db, select(User.id).where(User.id == user.id))
...
async def comp_forecast(major: str = "", periods: str = "",
    ...
    @cached(f"comp:{user.role}:{user.id}:{major}:{periods}")
    async def _compute():
        return await forecast(db, select(User.id).where(*conds) if conds else None)

    return {"code": 0, "data": {"list": await _compute()}}
